...
export function DevicesPage({ user }) {
  const admin = canRegisterDevice(user.role);
  const [devices, setDevices] = useState([]);
  const [catalog, setCatalog] = useState([]);
  const [alertedDeviceNames, setAlertedDeviceNames] = useState(new Set());
  const [total, setTotal] = useState(0);
  const [filters, setFilters] = useState({ search: "", device_type: "", location: "", status: "" });
  ...
  const visibleDevices = quickFilter === "alertas" ? devices.filter((device) => alertedDeviceNames.has(device.name)) : devices;
  const visibleTotal = quickFilter === "alertas" ? visibleDevices.length : total;
  ...
  useEffect(() => { fetchAlerts().then((alerts) => setAlertedDeviceNames(new Set(alerts.map((alert) => alert.deviceName)))).catch(() => setAlertedDeviceNames(new Set())); }, []);
  function updateFilter(field, value) { setQuickFilter("personalizado"); setPage(0); setFilters((current) => ({ ...current, [field]: value })); }
  function applyQuickFilter(nextFilter) {
    setQuickFilter(nextFilter); setPage(0);
    if (nextFilter === "todos") setFilters({ search: "", device_type: "", location: "", status: "" });
    if (nextFilter === "lab-a") setFilters({ search: "", device_type: "", location: "Lab-A", status: "" });
    if (nextFilter === "activos") setFilters({ search: "", device_type: "", location: "", status: "activo" });
    if (nextFilter === "alertas") setFilters({ search: "", device_type: "", location: "", status: "" });
  }
  ...
  return <section className="devices-page"><header className="devices-header"><div><h1>Dispositivos</h1><p>{visibleTotal} dispositivo{visibleTotal === 1 ? "" : "s"} registrado{visibleTotal === 1 ? "" : "s"}</p></div>{admin && <button type="button" className="device-primary-btn" onClick={() => setCreating(true)}><Plus size={18} /> Registrar dispositivo</button>}</header><section className="device-filter-bar" aria-label="Filtros de dispositivos"><label className="device-search"><Search size={17} aria-hidden="true" /><input value={filters.search} onChange={(event) => updateFilter("search", event.target.value)} placeholder="Buscar por nombre o ID..." /></label><select aria-label="Filtrar por tipo" value={filters.device_type} onChange={(event) => updateFilter("device_type", event.target.value)}><option value="">Todos los tipos</option>{filterOptions.types.map((type) => <option key={type} value={type}>{type}</option>)}</select><select aria-label="Filtrar por laboratorio" value={filters.location} onChange={(event) => updateFilter("location", event.target.value)}><option value="">Todos los laboratorios</option>{filterOptions.locations.map((location) => <option key={location} value={location}>{location}</option>)}</select><select aria-label="Filtrar por estado" value={filters.status} onChange={(event) => updateFilter("status", event.target.value)}><option value="">Todos los estados</option><option value="activo">Activos</option><option value="mantenimiento">Mantenimiento</option><option value="inactivo">Inactivos</option></select><div className="device-mobile-filter-chips"><button type="button" className={quickFilter === "todos" ? "is-active" : ""} onClick={() => applyQuickFilter("todos")}>Todos</button><button type="button" className={quickFilter === "lab-a" ? "is-active" : ""} onClick={() => applyQuickFilter("lab-a")}>Lab-A</button><button type="button" className={quickFilter === "activos" ? "is-active" : ""} onClick={() => applyQuickFilter("activos")}>Activos</button><button type="button" className={quickFilter === "alertas" ? "is-active" : ""} onClick={() => applyQuickFilter("alertas")}>Alertas</button></div></section>{error && <p className="device-form-error" role="alert">{error}</p>}<section className="devices-table-card">{loading ? <p className="device-muted">Cargando dispositivos...</p> : visibleDevices.length === 0 ? <div className="device-empty"><Thermometer size={28} /><p>No se encontraron dispositivos con esos filtros.</p>{admin && <button type="button" className="device-link-btn" onClick={() => setCreating(true)}>Registrar dispositivo</button>}</div> : <><div className="devices-table-wrap"><table><thead><tr><th>Nombre</th><th>Tipo</th><th>Laboratorio</th><th>Estado</th><th>Umbrales</th><th><span className="sr-only">Acciones</span></th></tr></thead><tbody>{visibleDevices.map((device) => <tr key={device.id}><td><strong>{device.name}</strong><small>{device.device_code}</small></td><td>{device.device_type}</td><td>{device.location}</td><td><span className={`device-status device-status--${device.status}`}>{device.status}</span></td><td>{device.thresholds.length}</td><td><DeviceActions device={device} {...actionProps} /></td></tr>)}</tbody></table></div><div className="device-mobile-list">{visibleDevices.map((device) => <MobileDeviceCard key={device.id} device={device} actionProps={actionProps} />)}</div><footer className="device-pagination"><span>Mostrando {visibleTotal ? page * PAGE_SIZE + 1 : 0}–{Math.min((page + 1) * PAGE_SIZE, visibleTotal)} de {visibleTotal} dispositivos</span><div><button type="button" disabled={page === 0} onClick={() => setPage((current) => current - 1)}>Anterior</button><span>{page + 1} / {totalPages}</span><button type="button" disabled={page + 1 >= totalPages} onClick={() => setPage((current) => current + 1)}>Siguiente</button></div></footer></>}</section>{creating && <DeviceFormModal onClose={() => setCreating(false)} onSaved={afterSave} />}{editing && <DeviceFormModal device={editing} onClose={() => setEditing(null)} onSaved={afterSave} />}{configuring && <ThresholdModal device={configuring} onClose={() => setConfiguring(null)} onSaved={afterSave} />}{selected && <DetailModal device={selected} admin={admin} onClose={() => setSelected(null)} onConfigure={() => { setConfiguring(selected); setSelected(null); }} />}</section>;
}
